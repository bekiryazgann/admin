...
@section('title', trans('Ürün Düzenle'))
@section('content')
    <div class="content-header row">
        <div class="content-header-left col-md-9 col-12 mb-2">
            <div class="row breadcrumbs-top">
                <div class="col-12">
                    <h2 class="content-header-title float-start mb-0">{{shorter(html_entity_decode($detail->name), 3)}}</h2>
                    <div class="breadcrumb-wrapper">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active">
                                {{shorter(html_entity_decode($detail->name), 3)}}
                            </li>
                        </ol>
                    </div>
                </div>
                        ...
                        <div class="row">
                                ...
                                <div class="mb-1 row">
                                    ...
                                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                                        <input type="text"
                                               ...
                                               id="prodcut-name"
                                               name="product_name"
                                               value="{{html_entity_decode($detail->name)}}"
                                ...
                                <div class="mb-1 row">
                                    ...
                                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                                        <select name="product_type" id="product-type" class="form-select">
                                            <option value="1" {{($detail->type == 1) ? 'selected' : ''}}>@trans(Fiziksel)</option>
                                            <option value="2" {{($detail->type == 2) ? 'selected' : ''}}>@trans(Dijital)</option>
                                        </select>
                            ...
                            <div class="col-12">
                                <div class="mb-1 row">
                                    ...
                                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                                        <input type="hidden" id="product-images" name="product_images" value='{{$detail->images}}'>
                                        <button type="button"
                                ...
                                <div class="mb-1 row">
                                    ...
                                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                                        <select class="form-select" id="product-brand" name="product_brand">
                                            ...
                                            <option value="0" {{((int)$detail->brand == 0) ? 'selected' : ''}}> - @trans(Yok) -</option>
                                            @foreach($brands as $brand)
                                                <option value="{{$brand->id}}" {{($detail->brand == $brand->id) ? 'selected' : ''}}>{{$brand->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                                    ...
                                                    @if($filter->status == 1)
                                                        <optgroup label="{{$filter->name}}">
                                                            @foreach(json_decode($filter->options, true) as $key => $options)
                                                                <option value="{{$filter->id . "@" . $key}}" {{in_array($filter->id . "@" . $key, json_decode($detail->tags, true)) ? 'selected' : ''}}>
                                                                    {{$filter->name}} → {{$options}}</option>
                                                            @endforeach
                                                        </optgroup>
                                                    @endif
                                                @endisset
                                    ...
                                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                                        <select class="tag-multi-select form-select" id="categories" multiple
                                            ...
                                            @foreach(listArray($categories) as $key => $category)
                                                <option value="{{$key}}" {{in_array($key, json_decode($detail->category)) ? 'selected' : ''}}>{!! $category['name'] !!}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="mb-1 row">
                                    ...
                                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                                        <input type="text"
                                               ...
                                               value="{{html_entity_decode($detail->short_description)}}"
                                ...
                                <div class="mb-1 row">
                                    ...
                                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                                        <textarea type="text"
                                                  ...
                                                  placeholder="@trans(Ürün Özellikleri ve Detaylı Açıklaması)">{{html_entity_decode($detail->long_description)}}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="mb-1 row">
                                        ...
                                        <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12 h-100">
                                            <textarea name="seo_description"
                                                      ...
                                                      style="resize: none"
                                                      id="seo-description"
                                                      placeholder="@trans(Açıklama)">{{$detail->seo_description}}</textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-4">

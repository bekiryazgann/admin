<div class="offcanvas-header">
    <h5 id="offcanvasEndLabel" class="offcanvas-title">Kampanya Düzenle</h5>
    <button type="button"
...
<div class="offcanvas-body">
    <form class="form form-vertical d-flex flex-column justify-content-between h-100 editCampaign"
                    ...
                    <div class="row">
                        <div class="col-12 mb-1">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="type" id="percentage_discount" value="1" {{($data['type'] == '1') ? 'checked' : ''}}>
                                <label class="form-check-label" for="percentage_discount">Yüzdelik İndirim</label>
                            </div>
                        </div>
                        <div class="col-12 mb-1">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="type" id="fixed_price" value="2" {{($data['type'] == '2') ? 'checked' : ''}}>
                                <label class="form-check-label" for="fixed_price">Sabit Fiyat</label>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="type" id="free_shipping" value="3" {{($data['type'] == '3') ? 'checked' : ''}}>
                    ...
                    <div class="row valuesContent">
                        ...
                        <h5 class="card-title mb-1 mt-2"> DEĞERLER </h5>
                            <div class="col-12">
                                <div class="mb-1">
                                    <label class="form-label"
                                           for="sale_fee">@trans(Kampanya İndirim Oranı) <span class="text-danger">*</span></label>
                        ...
                        <?php } if($data['type'] == '2'){ ?>
                        <h5 class="card-title mb-1 mt-2"> @trans(DEĞERLER) </h5>
                            <div class="col-12">
                                <div class="mb-1">
                                    <label class="form-label"
                        ...
                        <?php } if($data['type'] == '3'){ ?>
                            <input type="hidden" name="sale_fee" value="0">
                        <?php } ?>
                    </div>
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="form-check form-check-inline mb-1">
                    <input class="form-check-input"
                           type="checkbox"
                           name="valid_all_product"
                           id="valid_all_product"
                           value="1" {{($data['valid_all_product'] == '1') ? 'checked' : ''}}>
                ...
                <div class="mb-1 row">
                    ...
                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                        <select class="tag-multi-select form-select" id="valid_category" multiple name="valid_category[]">
                            ...
                            <option value="<?= $key ?>" {{ (($data['valid_category'] == null) ? '' : in_array($key, json_decode($data['valid_category'], true)))? 'selected' : ''}}><?= $category['name'] ?></option>
							<?php } ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-1 row">
                    ...
                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                        <select class="tag-multi-select form-select" id="valid_product" multiple name="valid_product[]">
                            ...
                            <option value="<?= $key ?>" {{ (($data['valid_product'] == null) ? '' : in_array($key, json_decode($data['valid_product'], true)))? 'selected' : ''}}><?= shorter($product->name, 4) ?></option>
							<?php } ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="mb-1 row">
                    ...
                    <div class="col-sm-12 col-lg-12 col-xs-12 col-12 col-md-12">
                        <select class="tag-multi-select form-select" id="valid_brand" multiple name="valid_brand[]">
                            ...
                            <option value="<?= $key ?>" {{ (($data['valid_brand'] == null) ? '' : in_array($key, json_decode($data['valid_brand'], true)))? 'selected' : ''}}><?= $brand->name ?></option>
							<?php } ?>
                        </select>
                    </div>
                </div>
...
<script>
    ...
    $('.editCampaign').on('submit', function(e){
        ...
        $.post(url + "api/edit-campaign", $(this).serialize() + '&id={{$data['id']}}', function(response){
            if (response.error) {
    ...
    {!! ($data['valid_all_product'] == '1') ? "$('.valid_all_product_hide').hide()" : "" !!}
</script>
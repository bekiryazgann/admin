let editMenuCanvas = $('#edit-menu-canvas');
let editMenuBsCanvas = new bootstrap.Offcanvas(editMenuCanvas);
...
let addToMenu = function () {
    ...
    nestableList.append(
        ...
        data-deleted="0">
            ...
            <span class="button-delete btn btn-danger btn-xs pull-right" data-owner-id="${newId}">
                <i class="fas fa-trash" aria-hidden="true"></i>
            </span>
            <span class="button-edit btn btn-primary btn-xs pull-right" data-owner-id="${newId}">
...
$(function () {
    ...
    let addMenuCanvas = $('#add-menu-canvas');
    let addMenuBsCanvas = new bootstrap.Offcanvas(addMenuCanvas);
    ...
    $("#menu-add").submit(function (e) {
        e.preventDefault();
        addToMenu();
        addMenuBsCanvas.hide();
    });

    addMenuCanvas.on('hide.bs.offcanvas', e => {
        setTimeout(e => {
            document.querySelectorAll('#menu-add input').forEach(element => {
                element.value = '';
            })
        }, 200)
    })
